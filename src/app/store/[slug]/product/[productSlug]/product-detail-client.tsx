...
export default function DetailProductPage({
  ...
  const { data: fetchedItems, isLoading: isLoadingItemsQuery } =
    useGetPublicStoreItemsQuery(storeSlug, { skip: !storeSlug });
  const { data: fetchedStore } = useGetPublicStoreQuery(storeSlug, {
    skip: !storeSlug,
  });
        ...
        <section className="mx-auto my-12 max-w-7xl px-4 sm:px-6 md:px-12 lg:px-20">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {relatedStoreItems.map((item) => (
              <MenuProductCard
                key={item.id}
                item={toMenuItem(item, currency, storeDetail?.displayExchangeRate)}
              />
            ))}
          </div>
        </section>
