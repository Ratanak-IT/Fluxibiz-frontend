...
function TelegramStoreContent() {
  ...
  useEffect(() => {
    if (!businessId) {
      setError("Missing business ID in URL (?businessId=...)");
      setLoading(false);
      return;
    }
    ...
    const fetchData = async () => {
      try {
        ...
        const [storeRes, itemsRes] = await Promise.all([
          fetch(`${baseUrl}/public/stores/${businessId}`),
          fetch(`${baseUrl}/public/stores/${businessId}/items`),
        ]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white space-y-3">
        ...
        <p className="text-xs text-slate-400">Loading Telegram Storefront...</p>
      </div>
    );
  }
