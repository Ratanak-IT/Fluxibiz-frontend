...
import { CheckCircle2, Loader2, QrCode, ExternalLink, ShieldCheck } from "lucide-react";
...
export default function MessengerCheckoutPage({
    ...
    useEffect(() => {
        ...
        async function fetchOrderDetails() {
            try {
                ...
                const res = await fetch(`${apiBase}/api/v1/public/orders/${orderId}/status`);
                if (res.ok) {
    ...
    return (
        <div className="flex min-h-screen flex-col bg-slate-50 p-4 font-sans text-slate-900">
            ...
            <Script src="https://connect.facebook.net/en_US/messenger.Extensions.js" strategy="lazyOnload" />

            {/* 📍 Progress Bar Step Indicator */}
            <div className="mb-4 flex items-center justify-between rounded-2xl border border-slate-100 bg-white p-3 shadow-sm">
                <StepBadge active={step === "PAYMENT"} completed={step === "PROCESSING" || step === "DONE"} stepNumber="1" label="ទូទាត់" />
                <div className={`h-0.5 flex-1 mx-2 ${step !== "PAYMENT" ? "bg-emerald-500" : "bg-slate-200"}`} />
                <StepBadge active={step === "PROCESSING"} completed={step === "DONE"} stepNumber="2" label="ផ្ទៀងផ្ទាត់" />
                <div className={`h-0.5 flex-1 mx-2 ${step === "DONE" ? "bg-emerald-500" : "bg-slate-200"}`} />
                <StepBadge active={step === "DONE"} completed={step === "DONE"} stepNumber="3" label="ជោគជ័យ" />
            </div>
            ...
            {step === "PAYMENT" && (
                <div className="flex flex-1 flex-col justify-between rounded-3xl border border-slate-100 bg-white p-6 text-center shadow-sm">
                    <div>
                        ...
                        <h1 className="text-lg font-bold text-slate-800">ទូទាត់ប្រាក់តាម KHQR</h1>
                        <p className="mt-1 text-3xl font-black text-emerald-600">
                            ${orderData.total.toFixed(2)} {orderData.currency !== "USD" ? orderData.currency : ""}
                        </p>
            ...
            {step === "PROCESSING" && (
                <div className="flex flex-1 flex-col items-center justify-center rounded-3xl border border-slate-100 bg-white p-8 text-center shadow-sm">
                    ...
                    <h2 className="text-xl font-bold text-slate-900">កំពុងផ្ទៀងផ្ទាត់ការទូទាត់...</h2>
            ...
            {step === "DONE" && (
                <div className="flex flex-1 flex-col items-center justify-center rounded-3xl border border-slate-100 bg-white p-8 text-center shadow-sm">
                    ...
                    <h2 className="text-2xl font-bold text-slate-900">ទូទាត់ប្រាក់ជោគជ័យ!</h2>
                    <p className="mt-2 text-sm text-slate-500">
...
function StepBadge({ active, completed, stepNumber, label }: any) {
    return (
        <div className="flex items-center gap-2">
            ...
            <span className={`text-xs font-semibold ${active || completed ? "text-slate-900" : "text-slate-400"}`}>
                {label}
            </span>
        </div>
    );
