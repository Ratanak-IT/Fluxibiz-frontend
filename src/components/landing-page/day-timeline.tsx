...
function StepCard({ step, index }: { step: Step; index: number }) {
  ...
  return (
    <ScrollReveal
      ...
      delay={index * 80}
      className={
        alignRight ? "sm:ml-auto sm:w-[54%]" : "sm:mr-auto sm:w-[54%]"
      }
    >
      <Card className="group relative overflow-hidden rounded-3xl border-[0.5px] border-hairline bg-white p-7 transition-all duration-500 ease-out dark:bg-card sm:p-8">
        <div
          className={`flex flex-col items-stretch gap-6 sm:flex-row ${
            alignRight ? "sm:flex-row-reverse" : ""
          }`}
        >
          <div className="relative flex min-h-40 w-full shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-white p-3 dark:bg-card sm:min-h-50 sm:w-1/2">
            <Image
          ...
          <div className="flex flex-1 flex-col justify-center text-center sm:text-left">
            ...
            <p className="mt-3 text-base leading-relaxed text-muted-foreground dark:text-muted-foreground sm:text-lg">
              {step.body}
            </p>
          </div>
        </div>
...
export function DayTimeline() {
  const t = useTranslations("Feature.timeline");

  const steps: Step[] = [
    {
      title: t("steps.open.title"),
      body: t("steps.open.description"),
      illustration: "/image/landing/open-door.png",
      tone: "brand",
    },
    {
      title: t("steps.sell.title"),
      body: t("steps.sell.description"),
      illustration: "/image/landing/sale.png",
      tone: "amber",
    },
    {
      title: t("steps.stock.title"),
      body: t("steps.stock.description"),
      illustration: "/image/landing/stock.png",
      tone: "rose",
    },
    {
      title: t("steps.business.title"),
      body: t("steps.business.description"),
      illustration: "/image/landing/dashboard.png",
      tone: "brand",
    },
    {
      title: t("steps.clarity.title"),
      body: t("steps.clarity.description"),
      illustration: "/image/landing/cash-flow.png",
      tone: "amber",
    },
    {
      title: t("steps.close.title"),
      body: t("steps.close.description"),
  ...
  return (
    <Section className="overflow-hidden bg-[#f5f5f5] py-24 dark:bg-background md:py-32">
      <ScrollReveal className="text-center">
        <RuledEyebrow>{t("eyebrow")}</RuledEyebrow>

        <h2 className="mt-5 font-display text-4xl font-extrabold text-brand-deep dark:text-primary sm:text-5xl lg:text-6xl">
          {t("title")}
        </h2>

        <p className="mt-4 text-base text-muted-foreground dark:text-muted-foreground sm:text-xl">
          {t("description")}
        </p>
      </ScrollReveal>
