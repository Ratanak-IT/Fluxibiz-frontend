...
const FLOAT_DOTS = [
  { top: "8%", left: "6%", size: "size-2", color: "bg-brand/30", anim: "hero-float-sm", depth: 1.4 },
  { top: "18%", left: "42%", size: "size-1.5", color: "bg-amber/40", anim: "hero-float", depth: 0.8 },
  { top: "6%", left: "68%", size: "size-2.5", color: "bg-brand/25", anim: "hero-float-slow", depth: 1.8 },
  { top: "34%", left: "12%", size: "size-1.5", color: "bg-amber/35", anim: "hero-float-slow", depth: 1 },
  { top: "60%", left: "4%", size: "size-2", color: "bg-brand/20", anim: "hero-float", depth: 1.6 },
  { top: "72%", left: "36%", size: "size-1.5", color: "bg-brand/30", anim: "hero-float-sm", depth: 0.7 },
  { top: "50%", left: "88%", size: "size-2", color: "bg-amber/30", anim: "hero-float-slow", depth: 1.3 },
  { top: "80%", left: "70%", size: "size-1.5", color: "bg-brand/25", anim: "hero-float", depth: 0.9 },
  { top: "14%", left: "92%", size: "size-2", color: "bg-brand/20", anim: "hero-float-sm", depth: 1.5 },
  { top: "88%", left: "18%", size: "size-2.5", color: "bg-amber/25", anim: "hero-float", depth: 1.1 },
] as const;

function FloatingDots({ containerRef }: { containerRef: RefObject<HTMLDivElement | null> }) {
  return (
    <div ref={containerRef} className="pointer-events-none absolute inset-0 -z-10" aria-hidden>
      {FLOAT_DOTS.map((dot, i) => (
        ...
        >
          <span className={`block rounded-full ${dot.size} ${dot.color} ${dot.anim}`} />
        </div>
      ))}
    </div>
  );
...
function HeroWave() {
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-0" aria-hidden>
      <svg viewBox="0 0 1440 220" preserveAspectRatio="none" className="h-32 w-full md:h-64">
        <path
        ...
        <div className="relative">
          ...
          <h1 className="mt-6 font-display text-[2.85rem] font-extrabold leading-[1.02] tracking-tight text-brand-ink md:text-[4rem]">
            ...
            <span className="relative inline-block">
              <em className="font-serif italic text-brand">one screen</em>
              <svg
                viewBox="0 0 170 14"
