...
export function StoreCardComponent({ store }: StoreCardComponentProps) {
  const { name,  category,description, location, hours, image, discountLabel, isOpen } =
    store;

  return (
    <div className="relative mx-auto w-68 pt-0 mt-3 overflow-hidden">
      {/* Store Image */}
      <div className="relative grow h-38 w-full ">
        <Image
          src={image}
          fill
          alt={`${name} cover`}
